...
static void exclude_tables2(
    table *victim,
    vector<table *> &target_tables,
    multimap<sqltype*, table*> &target_t_with_c_of_type,
    vector<named_relation *> &excluded_tables,
    vector<pair<sqltype*, table*>> &excluded_t_with_c_of_type)
{
...
shared_ptr<table_ref> table_ref::factory(prod *p, bool no_join) {
    try {
        if (p->level < 3 + d6() && p->level < max_level) {
            // fast_path
            if (rounds % 4 == 0) {
                make_shared<table_or_query_name>(p);
            }
            if (d6() > 5 && p->level < d6() && schema::target_ddbms != "vitess") //bug 19536
...
table_or_query_name::table_or_query_name(prod *p, bool only_base_table) : table_ref(p) {
    // this->scope->relation_id = unique_relation_id();
    if (only_base_table == false){
        // pick a table from a unused group
        auto table_iter = random_pick(scope->used_table_of_group);
        t = table_iter;
    }
    else {
        vector<table*> base_tables;
        for (auto t : scope->used_table_of_group) {
            if (t->is_base_table)
...
void table_or_query_name::out(std::ostream &out) {
    string table_name;
    if(""== table_name_after_action){
        table_name = t->ident();
    } else {
        table_name = table_name_after_action;
    }

    if (refs[0]->ident() != table_name)
        out << table_name << " as " << refs[0]->ident();
    else
        out << table_name;
}
...
void joined_table::out(std::ostream &out) {
    ...
    out << *lhs;
    indent(out);
    // if (scope->schema->target_ddbms == "clickhouse")
    //     out << " global ";
    out << type << " join " << *rhs;
    indent(out);
    if (type != "cross"){
        if(joined_on_shard_key)
            out <<  "on (" << shard_key_condition << ")";
        else
            out << "on (" << *condition << ")";
    }
...
from_clause::from_clause(prod *p, bool only_base_table) : prod(p) {
    ...
    scope->nearest_from_clause_id = scope->nearest_from_clause_id + 1;
...
void where_clause::out(std::ostream &out){
    if(assigned_shard){
        out<<"(" << *assigned_shard_name << " = " << *assigned_shard_value << ") and (" << *search <<")";
    } else {
        out << *search;
    }
}
...
void query_spec::out(std::ostream &out) {
    out << "select " << hint << set_quantifier << " " << *select_list;
    indent(out);
    out << *from_clause;
    indent(out);
    out << "where " << *search;
...
query_spec::query_spec(prod *p, struct scope *s, 
  ...
  prod(p), myscope(s)
{
    relation_id = unique_relation_id();
    ...
    search = make_shared<struct where_clause>(this); 
  ...
  prod(p), myscope(s)
{
    ...
    search = make_shared<struct where_clause>(this,where_search); 
  ...
  prod(p), myscope(s)
{
    ...
    from_clause = make_shared<struct from_clause>(this, from_table);
    search = make_shared<struct where_clause>(this, make_shared<struct comparison_op>(this, target_op, left_operand, right_operand));
    select_list = make_shared<struct select_list>(this, &from_clause->reflist.back()->refs, (vector<sqltype *> *)NULL, true);
}
...
{
    relation_id = unique_relation_id();
    relation_id_for_victim_table = unique_relation_id();
    // victim_relation_id = unique_relation_id();
    scope = &myscope;
    scope->tables = s->tables;
...
delete_stmt::delete_stmt(prod *p, struct scope *s, table *v)
  : modifying_stmt(p,s,v) {
    ...
    exclude_tables2(victim, scope->used_table_of_group, 
                    scope->schema->tables_with_columns_of_type, 
                    excluded_tables, 
                    excluded_t_with_c_of_type);
    ...
    for (auto v : views) {
        ...
        exclude_tables2(v, scope->used_table_of_group, scope->schema->tables_with_columns_of_type, 
                        excluded_tables, excluded_t_with_c_of_type);
    }
...
void delete_stmt::out(ostream &out) {
    string victim_name;
    if(mutated){
        victim_name = table_name_after_action;
    }else{
        victim_name = victim->ident();
    }
    if (schema::target_dbms == "clickhouse") 
        out << "alter table " << victim_name << " delete";
    else 
        out << "delete from " << victim_name;
    indent(out);
...
set_list::set_list(prod *p, table *target, vector<string> exclude_cols) : prod(p), myscope(p->scope)
{
    // update's scope might change (e.g. add victim table to scope->refs), 
    // use the current version only
    scope = &myscope;
    ...
    do {
        for (auto col : target->columns()) {
            if (find(exclude_cols.begin(), exclude_cols.end(), col.name) != exclude_cols.end())
                continue;
...
update_stmt::update_stmt(prod *p, struct scope *s, table *v)
  : modifying_stmt(p, s, v) {
    ...
    if(// this->scope->schema->target_ddbms == "citus" &&
        this->scope->schema->ddss.count(victim->name) > 0 ){
        auto& dds = this->scope->schema->ddss[victim->name];
        string dkey = dds.params["dkey"];
        if(dkey != "")
            set_list = make_shared<struct set_list>(this, victim, vector<string>{dkey});
        else
...
// partition_by_clause::partition_by_clause(prod *p, table *t) : prod(p) 
// {
//     // 
//     type = static_cast<partition_type>(dx(2) - 1);
    
//     // 1-3
//     auto columns = t->columns();
//     int partition_key_count = dx(min(3, (int)columns.size()));
    
//     set<string> selected_cols;
//     while (selected_cols.size() < partition_key_count) {
//         auto &col = random_pick(columns);
//         // (vkeypkey)
//         // 
//         if (col.name != VKEY_IDENT && col.name != PKEY_IDENT && selected_cols.count(col.name) == 0) {
//             selected_cols.insert(col.name);
//             partition_columns.push_back(col.name);
//         }
//     }
...
//             switch (type) {
//                 case RANGE:
//                     bound = "PARTITION " + to_string(i) + " FOR VALUES FROM (";
//                     for (size_t j = 0; j < partition_columns.size(); j++) {
//                         bound += to_string(i * 100);  // 
//                         if (j < partition_columns.size() - 1) bound += ", ";
//                     }
//                     bound += ") TO (";
//                     for (size_t j = 0; j < partition_columns.size(); j++) {
//                         bound += to_string((i + 1) * 100);
//                         if (j < partition_columns.size() - 1) bound += ", ";
//                     }
//                     bound += ")";
//                     break;
...
//                 case LIST:
//                     bound = "PARTITION " + to_string(i) + " FOR VALUES IN (";
//                     for (int j = 0; j < dx(3) + 1; j++) {
//                         bound += to_string(j + i * 10);
//                         if (j < dx(3)) bound += ", ";
//                     }
//                     bound += ")";
//                     break;
...
//                 default: break;
//             }
//             partition_bounds.push_back(bound);
//         }
//     }
// }

// void partition_by_clause::out(std::ostream &out) 
// {
//     out << "PARTITION BY ";
//     switch (type) {
//         case RANGE: out << "RANGE"; break;
//         case LIST: out << "LIST"; break;
//         case HASH: out << "HASH"; break;
//     }
    
//     out << " (";
//     for (size_t i = 0; i < partition_columns.size(); i++) {
//         out << partition_columns[i];
//         if (i < partition_columns.size() - 1) out << ", ";
//     }
//     out << ")";
...
//             out << partition_bounds[i];
//             if (i < partition_bounds.size() - 1) out << ",\n";
//         }
//     }
// }
...
: prod(parent), myscope(s)
{
    ...
    for (int i = 0; i < column_num; i++) {
        ...
        if(this->scope->schema->target_ddbms != "ss")
        ...
        if (d6() == 1 && s->schema->target_ddbms == "tidb") 
            constraint_str += "not null ";
        if (d12() == 1 && s->schema->target_ddbms == "tidb")
...
create_table_stmt::create_table_stmt(prod *parent, struct scope *s, shared_ptr<struct table> t)
: prod(parent), myscope(s)
{
    scope = &myscope;
    scope->tables = s->tables;
    created_table = t;
    //new name
    created_table -> name = unique_table_name(scope);
    //old constraints
    for(auto & constraint : created_table->constraints){
        constraints.push_back(constraint);
    }
}

create_table_stmt::create_table_stmt(prod *parent, struct scope *s, shared_ptr<struct create_table_stmt> create_table)
:prod(parent), myscope(s){
    scope = &myscope;
    scope->tables = s->tables;
    // create table
    ...
    table_name = unique_table_name(scope);
    created_table = make_shared<struct table>(table_name, "main", true, true);
    bool has_option = create_table->has_option;
    string table_option = create_table->table_option;
    bool has_check = create_table->has_check;
    shared_ptr<struct bool_expr> check_expr = create_table->check_expr; //maybe fault
    bool has_engine = create_table->has_engine;
    string table_engine = create_table->table_engine;
    bool has_primary_key = create_table->has_primary_key;
    string primary_key_str = create_table->primary_key_str;

    for(auto col : create_table->created_table->columns()){
        column create_column(col.name, col.type);
        created_table->columns().push_back(create_column);
        constraints.push_back("");
    }
}
...
void create_table_stmt::out(std::ostream &out)
{
    ...
    // if (data_distribution_strategy) {
    //     out << *data_distribution_strategy;
    // }
}
...
create_index_stmt::create_index_stmt(prod *parent, struct scope *s, table *target_table)
: prod(parent), myscope(s)
{
    scope = &myscope;
    ...
    if (target_table && target_table->is_base_table) {
        indexed_table = target_table;
    }
...
{
    out << "ANALYZE ";
    if (scope->schema->target_dbms == "tidb"||scope->schema->target_ddbms == "tidb")
        out << "TABLE ";
    out << target;
}
...
shared_ptr<prod> basic_dml_statement_factory(struct scope *s, table *target_table)
{
    try { // only use insert_stmt to add data to target table
        ...
        else if (s->schema->target_ddbms == "tidb" && choice <= 2) // CLICKHOUSE does not support CREATE INDEX
            return make_shared<create_index_stmt>((struct prod *)0, s, target_table);
        else 
            return make_shared<insert_stmt>((struct prod *)0, s, target_table, true);

    } catch (runtime_error &e) {
        cerr << "err: " << e.what() << endl;
        cerr << "catch a runtime error in " << __FUNCTION__  << endl;
        return basic_dml_statement_factory(s, target_table);
    }
}
