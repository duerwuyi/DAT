...
std::string get_cluster_name(std::string path) {
    std::ifstream in(path);
    if (!in.is_open()) {
        throw std::runtime_error("cannot open cluster config file: " + path);
    }
    std::string name;
    std::getline(in, name);
    trim_inplace(name);
    ...
    for (unsigned char ch : name) {
        if (!(std::isalnum(ch) || ch == '_' || ch == '-')) {
            throw std::runtime_error("invalid cluster name in file: " + name);
        }
    }
    return name;
}
...
bool execute_clickhouse_query(string* db,string* ip, 
                    ...
                    bool print = false)
{
    ...
    if(print){
        ...
        cout<<"clickhouse test: "<< std::put_time(&now_tm, "%Y-%m-%d %H:%M:%S")
                    << '.' << std::setfill('0') << std::setw(5) << now_ms.count() << ", port: " << to_string(port) << std::endl;
        cout<<query<<endl;
    }
...
clickhouse_connection::clickhouse_connection(string db,string ip, int port,bool distributed)
{
    test_db = db;
    test_ip = ip;
    test_port = port;
    ...
    if(cluster != ""){
        create_sql += " on cluster " + cluster + ";";
    }else{
        create_sql += ";";
    }
    auto succeed = execute_clickhouse_query(NULL, &test_ip, test_port, create_sql, NULL, error, NULL,false);
    if (!succeed) {
        throw runtime_error("CONNECTION ERROR [" + error + "]");
    }
}

map<string, string> read_dkeys(){
    ...
    while (std::getline(in, line)) {
        ...
        std::string dkey;
        if (!(iss >> table_name)) continue;  
        if (!(iss >> dkey)) {
            throw std::runtime_error("bad config clickhouse_dkey.log: missing dkey");
        }
        res[table_name] = dkey;
    }
    return res;
...
schema_clickhouse::schema_clickhouse(string db,string ip, int port,bool distributed)
: clickhouse_connection(db,ip, port,distributed)
{
    vector<vector<string>> output;
    string error;
    auto dkey_map = read_dkeys();
    // cerr << "Loading tables...";
    // string query_tables = CLICKHOUSE_GET_TABLES(test_db);
    std::string query_tables = R"SQL(WITH
  engine_full AS ef,
  extractAll(ef, '''([^'']*)''') AS qargs
SELECT
  name,
  engine,
  partition_key,
  -- First three arguments: cluster / remote_db / remote_table
  arrayElement(qargs, 1) AS dist_cluster,
  arrayElement(qargs, 2) AS remote_db,
  arrayElement(qargs, 3) AS remote_table,
  -- Fourth argument: sharding key expression, possibly unquoted, e.g. rand(), cityHash64(id)
  replaceRegexpOne(
    ef,
    '^Distributed\\(''[^'']*''\\s*,\\s*''[^'']*''\\s*,\\s*''[^'']*''\\s*,\\s*(.*)\\)$',
    '\\1'
  ) AS sharding_key_expr,
  engine_full
FROM system.tables
    )SQL";
    query_tables += " WHERE database = '"+test_db+"';";
    // rand(), cityHash64(id)
    auto succeed = execute_clickhouse_query(&test_db,&test_ip, test_port, query_tables, &output, error, NULL);
    if (succeed == false)
        throw runtime_error("ERROR IN LOADING TABLES [" + error + "]");
    for (auto& row : output) {
        if(row[0][0]!='t') continue; 
        clickhouse_table tab(row[0], "main", true, true);
        tab.engine = row[1];
        tab.partition_key = row[2];
        tab.dist_cluster = row[3];
        tab.sharding_key_expr = dkey_map[row[0]];
        tables.push_back(tab);
        dds d;
        d.params["victim"] = tab.name;
        d.params["type"] = to_string(tab.get_type());
        d.params["colocate"] = tab.sharding_key_expr == "colocated_key" ? "1" : "-1";
        if(tab.sharding_key_expr != ""){
            d.params["dkey"] = tab.sharding_key_expr;
        }
        this -> register_dds(make_shared<clickhouse_table>(tab), d);
    }
    // cerr << "done." << endl;

    // cerr << "Loading views...";
    // string query_views = CLICKHOUSE_GET_VIEWS(test_db);
    // succeed = execute_clickhouse_query(&test_db,&test_ip, test_port, query_views, &output, error, NULL);
    // if (succeed == false)
    ...
    for (auto& t : tables) {
        ...
        succeed = execute_clickhouse_query(&test_db,&test_ip, test_port, query_columns, &output, error, NULL);
        if (succeed == false)
        ...
        for (auto& row : output) {
            ...
            // if (auto tab = dynamic_cast<clickhouse_table*>(&t)) {
            //     if (tab->partition_key == row[0]) {
            //         tab->partition_column = stored;
            //     }
            // }
        }
    }
    ...
    supported_join_op.push_back("global right outer");
    supported_join_op.push_back("global full outer");
    supported_join_op.push_back("global inner");
    supported_join_op.push_back("global left outer");
...
{
    string error;
    auto succeed = execute_clickhouse_query(&test_db,&test_ip, test_port, stmt, output, error, env_setting_stmts,true);
    if (!succeed) {
...
void dut_clickhouse::reset()
{
    ...
    if(cluster != ""){
        drop_sql += " on cluster " + cluster + ";";
    }else{
        drop_sql += ";";
    }
    auto succeed = execute_clickhouse_query(NULL,&test_ip, test_port, drop_sql, NULL, error, NULL,true);
    if (!succeed) {
    ...
    succeed = execute_clickhouse_query(NULL,&test_ip, test_port, create_sql, NULL, error, NULL,true);
    if (!succeed) {
...
void dut_clickhouse::get_content(vector<string>& tables_name, map<string, vector<vector<string>>>& content)
{
    for (auto& table : tables_name) {
        ...
        auto succeed = execute_clickhouse_query(&test_db,&test_ip, test_port, query, &table_content, error, NULL,true);
        if (!succeed) {
...
dut_clickhouse::dut_clickhouse(string db,string ip, int port, string backup_file,bool distributed)
    : clickhouse_connection(db,ip, port,distributed)
{
