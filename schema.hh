...
struct schema {
    ...
    void fill_table_versions() {
        for (auto &t : tables) {
            if(t.is_base_table == false || t.is_insertable == false) continue;
            if(t.get_id_from_name() == -1) continue;
            table_versions.insert(std::make_shared<table>(t));
        }
    }
    ...
    void fill_scope(struct scope &s) {
        ...
        for (auto it = table_versions.table_id_with_versions.begin(); it != table_versions.table_id_with_versions.end(); ) {
            int key = it->first;
        
            auto range = table_versions.table_id_with_versions.equal_range(key);
            
            if(random_init){
                s.used_table_of_group.push_back(random_pick(range)->second.get());
            }
            else{
                auto res = range.first;
                int best = res->second->get_id_from_name();
                for (auto jt = std::next(range.first); jt != range.second; ++jt) {
                    int cur = jt->second->get_id_from_name();
                    if (cur < best) {
                        best = cur;
                        res = jt;
                    }
                }
                s.used_table_of_group.push_back(res->second.get());
            }
            it = range.second;
        }
        cout<< "tables selected: ";
        for(auto& t:s.used_table_of_group){
            cout << t->ident()<<" ";
        }
        cout<<endl;
        s.schema = this;
    ...
    virtual void register_dds(shared_ptr<table> t, dds d) {
        ddss[t->name] = d;
    }
    ...
    schema() { }
    virtual void refresh() {cout << "base refresh" << endl;};
