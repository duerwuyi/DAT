...
mysql_connection::mysql_connection(string db, string ip, unsigned int port)
{
    test_db = db;
    test_ip = ip;
    test_port = port;
    ...
    if (mysql_real_connect(&mysql, test_ip.c_str(), "root", NULL, test_db.c_str(), test_port, NULL, 0)) 
        return; // success
    ...
    if (!mysql_real_connect(&mysql, test_ip.c_str(), "root", NULL, NULL, test_port, NULL, 0))
        throw std::runtime_error(string(mysql_error(&mysql)) + "\nLocation: " + debug_info);
...
schema_mysql::schema_mysql(string db, string ip, unsigned int port)
...
dut_mysql::dut_mysql(string db, string ip, unsigned int port)
  : mysql_connection(db,ip, port)
{
    sent_sql = "";
    has_sent_sql = false;
...
{
    dut_mysql another_dut(test_db, test_ip, test_port);
...
std::pair<std::string, std::string> split_at_first_newline1(const std::string& s) {
    size_t pos = s.find('\n');
...
{
    auto backup_name = "/tmp/"+ test_db + "_" + test_ip + "_" + to_string(test_port) + "_schema_bk.sql";
    string mysql_dump = "mysqldump -d -h " + test_ip + " -P " + to_string(test_port) + " -u root " + test_db + " > " + backup_name + " --set-gtid-purged=off";
    int ret = system(mysql_dump.c_str());
    if (ret != 0) {
        cerr << "backup fail in dut_mysql::backup 1!!" << endl;
        throw std::runtime_error("backup fail in dut_mysql::backup 1"); 
    }
    backup_name = "/tmp/"+ test_db + "_" + test_ip + "_" + to_string(test_port) + "_data_bk.sql";
    mysql_dump = "mysqldump -t -h " + test_ip + " -P " + to_string(test_port) + " -u root " + test_db + " > " + backup_name + " --set-gtid-purged=off";
    ret = system(mysql_dump.c_str());
    if (ret != 0) {
        cerr << "backup fail in dut_mysql::backup 2!!" << endl;
        throw std::runtime_error("backup fail in dut_mysql::backup 2"); 
    }
}
...
void dut_mysql::load_backup(void){
    load_backup_schema();
    load_single_backup_data();
}

void dut_mysql::load_backup_schema(void){
    string bk_file = "/tmp/"+ test_db + "_" + test_ip + "_" + to_string(test_port) + "_schema_bk.sql";
    if (access(bk_file.c_str(), F_OK ) == -1) 
        return;
    
    mysql_close(&mysql);
    
    string mysql_source = "mysql -h " + test_ip + " -P " + to_string(test_port) + " -u root -D " + test_db + " < " + bk_file;
    cout << mysql_source << endl;
...
void dut_mysql::load_single_backup_data(void){
    string bk_file = "/tmp/"+ test_db + "_" + test_ip + "_" + to_string(test_port) + "_data_bk.sql";
    if (access(bk_file.c_str(), F_OK ) == -1) 
        return;
