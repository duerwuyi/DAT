...
struct table_ref : prod {
  ...
  table_ref(prod *p) : prod(p) {relation_id = unique_relation_id(); }
  virtual ~table_ref() { }
};
...
struct table_or_query_name : table_ref {
    ...
    string table_name_after_action;
...
struct where_clause : bool_expr {
    bool assigned_shard = false;

    bool victim_assigned_shard = false;

    shared_ptr<bool_expr> search;
    shared_ptr<value_expr> assigned_shard_name;
    shared_ptr<value_expr> assigned_shard_value;
    virtual void out(std::ostream &out);
    where_clause(prod *p): bool_expr(p) {
      search = bool_expr::factory(this); 
    };
    where_clause(prod *p, shared_ptr<bool_expr> b): bool_expr(p) {
      search = b; 
    };
...
struct from_clause : prod {
    ...
    std::vector<named_relation *> outer_refs;
    int id;
    virtual void out(std::ostream &out);
...
struct query_spec : prod {
    ...
    shared_ptr<struct select_list> select_list;
    shared_ptr<where_clause> search;
...
struct modifying_stmt : prod {
    ...
    struct scope myscope;
    int relation_id;
    int relation_id_for_victim_table;
    bool mutated = false;
    string table_name_after_action = "";
    // int victim_relation_id;
...
struct set_list : prod {
    ...
    set_list(prod *p, table *target,vector<string> exclude_cols);
    virtual ~set_list() {  }
    virtual void out(std::ostream &out);
    virtual void accept(prod_visitor *v) {
        v->visit(this);
...
// struct partition_by_clause : prod {
//     enum partition_type {
//         RANGE,
//         //LIST,
//         HASH
...
//     vector<string> partition_bounds;
    
//     partition_by_clause(prod *p, table *t);
//     virtual void out(std::ostream &out);
//     virtual void accept(prod_visitor *v) {
//         v->visit(this);
//     }
...
struct create_table_stmt: prod {
    ...
    // shared_ptr<struct create_dds> data_distribution_strategy;
    // bool has_partition = false;
    // shared_ptr<partition_by_clause> partition_clause;
    virtual void out(std::ostream &out);
    create_table_stmt(prod *parent, struct scope *s);
    create_table_stmt(prod *parent, struct scope *s, shared_ptr<struct table> t);
    create_table_stmt(prod *parent, struct scope *s, shared_ptr<struct create_table_stmt> create_table);
    
    // void create_data_distribution_strategy(){
    //   this->data_distribution_strategy = make_shared<struct create_dds>(this->created_table,this);
    // }
...
struct create_index_stmt: prod {
    ...
    bool is_unique;
    create_index_stmt(prod *parent, struct scope *s, table *target_table = NULL);
    virtual void out(std::ostream &out);
    virtual void accept(prod_visitor *v) {
        v->visit(this);
    }
...
shared_ptr<prod> basic_dml_statement_factory(struct scope *s, table *target_table = NULL);
