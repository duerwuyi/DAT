...
std::regex e_timeout("ERROR:  canceling statement due to statement timeout(\n|.)*");
std::regex e_syntax("ERROR:  syntax error at or near(\n|.)*");

#define debug_info (string(__func__) + "(" + string(__FILE__) + ":" + to_string(__LINE__) + ")")

bool has_types = false;
vector<pg_type *> static_type_vec;

bool has_operators = false;
vector<op> static_op_vec;

bool has_routines = false;
vector<routine> static_routine_vec;

bool has_routine_para = false;
map<string, vector<pg_type *>> static_routine_para_map;

bool has_aggregates = false;
vector<routine> static_aggregate_vec;

bool has_aggregate_para = false;
map<string, vector<pg_type *>> static_aggregate_para_map;
bool is_start = true;

void clear_postgres_static_cache()
{
    static_op_vec.clear();
    static_routine_vec.clear();
    static_routine_para_map.clear();
    static_aggregate_vec.clear();
    static_aggregate_para_map.clear();

    for (auto* t : static_type_vec)
        delete t;
    static_type_vec.clear();

    has_types = false;
    has_operators = false;
    has_routines = false;
    has_routine_para = false;
    has_aggregates = false;
    has_aggregate_para = false;
}
...
void log_query(const std::string& original_query) {
    ...
    log_file.open("query.log", std::ios_base::app); // 
...
void log(const std::string& original_query, const std::string& filepath){
    std::ofstream log_file;
    log_file.open(filepath, std::ios_base::app); // 
    std::string query = original_query; 
    if (log_file.is_open()) {
...
schema_pqxx::schema_pqxx(string db,string ip, unsigned int port, bool no_catalog)
    : pgsql_connection(db, ip, port)
{
    ...
    ||(schema == "columnar_internal")||(schema == "columnar")||(schema == "squeeze")
    ||(schema == "hint_plan")||(schema == "tbase_pg_proc")
    ||(table_name == "citus_tables")||(table_name == "citus_schemas")
    ||(table_name == "pg_stat_statements")||(table_name == "TBASE_SUBSCRIPTION")
    ||(table_name == "TBASE_SUBSCRIPTION_PARALLEL")||(table_name == "tbase_subscription")
    ||(table_name == "tbase_subscription_parallel")
    ))
            continue;
    //     if (!no_catalog && ((schema == "pg_catalog") || (schema == "information_schema")
    // ||(schema == "columnar_internal")||(schema == "columnar")||(schema == "squeeze")
    ...
    // ||(table_name == "tbase_subscription_parallel")
    // )){
    //     tables.push_back(table(table_name, schema,false,false));
    // }else
    ...
    for (auto t = tables.begin(); t != tables.end(); ++t) {
        string q("select attname, atttypid "
                ...
                "where not attisdropped "
                    ...
                    " 'xmin_gts', 'xmax_gts','shardid','xc_node_id') ");
    ...
    if (has_routines == false) {
        string load_routines_sql = 
            ...
            "where prorettype::regtype::text not in ('event_trigger', 'trigger', 'opaque', 'internal') "
                ...
                "AND proname NOT LIKE '%gin%'"
                "AND proname NOT LIKE '%bttext%'"
                "AND proname NOT LIKE '%inet%';";
...
pgsql_connection::pgsql_connection(string db, string ip, unsigned int port)
{    
    test_db = db;
    test_ip = ip;
    test_port = port;
    ...
    conn = PQsetdbLogin(ip.c_str(), to_string(port).c_str(), NULL, NULL, db.c_str(), PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
    ...
    cerr << "try to create database " << db << ", " << err << endl;
    conn = PQsetdbLogin(ip.c_str(), to_string(port).c_str(), NULL, NULL, "postgres", PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
    if (PQstatus(conn) != CONNECTION_OK) {
        string err = PQerrorMessage(conn);
...
dut_libpq::dut_libpq(string db,string ip, unsigned int port)
    : pgsql_connection(db,ip, port)
{
    //is_distributed = d;
...
static bool is_expected_error(string error)
{
    if (error.find("violates not-null constraint") != string::npos
        ...
        || error.find("not supported") != string::npos
        || error.find("only support") != string::npos
    ...
    if (output) {
        ...
        for (int i = 0; i < row_num; i++) {
            ...
            for (int j = 0; j < field_num; j++) {
                ...
                if (tmp == NULL)
                    str = "NULL";
                else if(stmt.find("EXPLAIN") != string::npos && j == 0) {
                    //do not process the "explain" result
                    str = tmp;
                }
...
void dut_libpq::load_db_setup(void){
    string pgsql_source = "psql -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " 
                + to_string(test_port) + " " + test_db
                + " -v ON_ERROR_STOP=0 -f db_setup.sql 1> /dev/null";
}
...
void dut_libpq::reset(void)
{
    if (conn) 
        PQfinish(conn);
    conn = PQsetdbLogin(test_ip.c_str(), to_string(test_port).c_str(), NULL, NULL, "postgres", PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
    if (PQstatus(conn) != CONNECTION_OK) {
        string err = PQerrorMessage(conn);
        cerr << err << " in " << debug_info << endl;
        throw runtime_error(err + " in " + debug_info);
    ...
    PQfinish(conn);
    conn = PQsetdbLogin(test_ip.c_str(), to_string(test_port).c_str(), NULL, NULL, test_db.c_str(), PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
    if (PQstatus(conn) != CONNECTION_OK) {
        string err = PQerrorMessage(conn);
        cerr << err << " in " << debug_info << endl;
        throw runtime_error(err + " in " + debug_info);
...
void dut_libpq::reset_without_force(void)
{
    if (conn) 
        PQfinish(conn);
    conn = PQsetdbLogin(test_ip.c_str(), to_string(test_port).c_str(), NULL, NULL, "postgres", PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
...
{
    string pgsql_dump = "pg_dump --schema-only -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " + 
                        to_string(test_port) + " " + test_db 
                        + " > " + POSTGRES_BK_FILE(test_db + to_string(test_port)+"schema");
    cerr<< pgsql_dump <<endl;
    int ret = system(pgsql_dump.c_str());
    if (ret != 0) {
        std::cerr << "backup fail \nLocation: " + debug_info << endl;
        throw std::runtime_error("backup fail \nLocation: " + debug_info); 
    }
    pgsql_dump = "pg_dump --data-only -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " + 
                        to_string(test_port) + " " + test_db 
                        + " > " + POSTGRES_BK_FILE(test_db + to_string(test_port)+"data");
    cerr<< pgsql_dump <<endl;
    ret = system(pgsql_dump.c_str());
    if (ret != 0) {
        std::cerr << "backup fail \nLocation: " + debug_info << endl;
        throw std::runtime_error("backup fail \nLocation: " + debug_info); 
    }
...
{
    cout<<"dut_libpq reset_to_backup"<<endl;
    reset();
    //string bk_file = POSTGRES_BK_FILE(test_db + to_string(test_port));
    ...
    string pgsql_source = "psql -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " 
                        + to_string(test_port) + " " + test_db + " < " 
                        + POSTGRES_BK_FILE(test_db + to_string(test_port)+"schema") + " 1> /dev/null";
    cerr<< pgsql_source <<endl;
    if (system(pgsql_source.c_str()) == -1)
        throw std::runtime_error(string("system() error, return -1") + "\nLocation: " + debug_info);

    pgsql_source = "psql -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " 
                        + to_string(test_port) + " " + test_db + " < " 
                        + POSTGRES_BK_FILE(test_db + to_string(test_port)+"data") + " 1> /dev/null";
    cerr<< pgsql_source <<endl;
    if (system(pgsql_source.c_str()) == -1)
        throw std::runtime_error(string("system() error, return -1") + "\nLocation: " + debug_info);
    conn = PQsetdbLogin(test_ip.c_str(), to_string(test_port).c_str(), NULL, NULL, test_db.c_str(), PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
...
void dut_libpq::load_backup_schema(void)
{
    PQfinish(conn);
    string pgsql_source = "psql -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " 
                        + to_string(test_port) + " " + test_db + " < " 
    ...
    conn = PQsetdbLogin(test_ip.c_str(), to_string(test_port).c_str(), NULL, NULL
    , test_db.c_str(), PG_USER_NAME.c_str(), PG_PASSWORD.c_str());
    if (PQstatus(conn) != CONNECTION_OK) {
        string err = PQerrorMessage(conn);
        throw runtime_error("[CONNECTION FAIL] " + err + " in " + debug_info);
    }
...
void dut_libpq::load_backup_data(void)
{
    PQfinish(conn);
...
void dut_libpq::load_single_backup_schema(void){
    PQfinish(conn);
    string pgsql_source = "psql -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " 
                        + to_string(test_port) + " " + test_db + " < " 
                        + POSTGRES_BK_FILE(test_db + "5432schema") + " 1> /dev/null";
    cerr<< pgsql_source <<endl;
    if (system(pgsql_source.c_str()) == -1)
        throw std::runtime_error(string("system() error, return -1") + "\nLocation: " + debug_info);
...
void dut_libpq::load_single_backup_data(void)
{
    PQfinish(conn);

    string pgsql_source = "psql -w -h " + test_ip + " -U " + PG_USER_NAME + " -p " 
                        + to_string(test_port) + " " + test_db + " < " 
                        + POSTGRES_BK_FILE(test_db + "5432data") + " 1> /dev/null";
    cerr<< pgsql_source <<endl;
    if (system(pgsql_source.c_str()) == -1)
        throw std::runtime_error(string("system() error, return -1") + "\nLocation: " + debug_info);
    conn = PQsetdbLogin(test_ip.c_str(), to_string(test_port).c_str(), NULL, NULL,
...
vector<string> dut_libpq::get_query_plan(const std::vector<std::vector<std::string>>& query_plan) {
    //  Distributed Subplan 
    const std::regex re_ds_truncate(R"((^\s*->\s*Distributed Subplan)\b.*$)");
    const std::regex re_node(R"(^\s*Node:.*$)");

    // 
    const std::regex re_paren(R"(\([^()]*\))");

    // 
    const std::regex re_spaces(R"(\s{2,})");

    //  "Key:" 
    const std::regex re_key_truncate(R"((^.*?\bKey:).*$)");

    //  "on"  "on"
    // \bon\b  WindowAgg 
    const std::regex re_on_truncate(R"((^.*?\bon)\b.*$)");
...
/*
Insert on t32  (cost=43.01..19434.30 rows=0 width=0)
  InitPlan 1
    ->  Limit  (cost=43.01..43.01 rows=1 width=4)
          ->  Sort  (cost=42.92..44.95 rows=810 width=4)
                Sort Key: t1.c1
                ->  Seq Scan on t1  (cost=0.00..18.10 rows=810 width=4)
  ->  Seq Scan on t14 ref_0  (cost=0.00..19391.29 rows=2 width=40)
        Filter: ((InitPlan 1).col1 = (SubPlan 3))
        SubPlan 3
          ->  Limit  (cost=43.05..43.06 rows=1 width=4)
                InitPlan 2
                  ->  Limit  (cost=23.85..23.85 rows=1 width=32)
                        ->  Sort  (cost=23.84..25.14 rows=520 width=32)
                              Sort Key: t11.c16
                              ->  Seq Scan on t11  (cost=0.00..15.20 rows=520 width=32)
                ->  Sort  (cost=19.20..19.70 rows=200 width=4)
                      Sort Key: (int4shr(ref_0.pkey, ref_1.c9)) DESC
                      ->  HashAggregate  (cost=15.70..18.20 rows=200 width=4)
                            Group Key: int4shr(ref_0.pkey, ref_1.c9)
                            ->  Result  (cost=0.01..14.75 rows=380 width=4)
                                  One-Time Filter: ((InitPlan 2).col1 < (ref_0.c42 || '2rn$n>CG[Siy3VQS<'::text))
                                  ->  Seq Scan on t10 ref_1  (cost=0.01..13.80 rows=380 width=4)
*/
